...
**Input modes** (`src/utils/input.ts`): `detectInputMode()` returns `'touch'` if `matchMedia('(pointer: coarse)')` matches, else `'keyboard'`. This is feature-detection (CSS-level), not UA sniffing — it correctly handles iPad+keyboard, Surface tablets, and laptops with touchscreens. Pacman always installs a `pointerdown`/`pointerup` swipe handler (30px threshold, dominant-axis wins) **alongside** the keyboard cursor reads, so both work on every device. The visible UI (CONTROLS panel, restart hint) is what flips between the two — only the user-facing instructions are mode-specific. The restart hint text is also `setInteractive`'d so a mouse user can click it; a SPACE keyboard binding is registered separately in `beginGame()`.
